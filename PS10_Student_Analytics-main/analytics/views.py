...
def _admin_dashboard_data():
    ...
    for student in User.objects.filter(role='student')[:20]:
        ...
        if marks:
            percentages = [(m.marks_obtained / m.subject.max_marks) * 100 if m.subject.max_marks > 0 else 0 for m in marks]
...
def _student_dashboard_data(user):
    ...
    for m in marks:
        ...
        if m.subject.max_marks > 0:
            pct = (m.marks_obtained / m.subject.max_marks) * 100
        else:
            pct = 0
        subject_perf[m.subject.name].append(float(pct))
...
def student_detail(request, pk):
    ...
    for sn, data in subject_analysis.items():
        ms = data['marks']
        percs = [(m.marks_obtained / m.subject.max_marks) * 100 if m.subject.max_marks > 0 else 0 for m in ms]
        data['avg'] = round(sum(percs) / len(percs), 1) if percs else 0
...
def subject_report(request, pk):
    ...
    for m in marks:
        ...
        if subject.max_marks > 0:
            pct = (m.marks_obtained / subject.max_marks) * 100
        else:
            pct = 0
        student_avgs[sid]['marks'].append(float(pct))
